...
_run_pipeline_parallel_tests = parse_flag_from_env("RUN_PIPELINE_PARALLEL_TESTS", default=True)
...
def is_pipeline_parallel_test(test_case):
    """
    Decorator marking a test as a pipeline parallel test. If RUN_PIPELINE_PARALLEL_TESTS is set to a falsy value,
    those tests will be skipped.
    """
    if not _run_pipeline_parallel_tests:
        return unittest.skip(reason="test is pipeline parallel test")(test_case)
    else:
        try:
            import pytest  # We don't need a hard dependency on pytest in the main library
        except ImportError:
            ...
            return pytest.mark.is_pipeline_parallel_test()(test_case)
